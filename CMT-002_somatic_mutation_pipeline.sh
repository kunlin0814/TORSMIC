...
annovar_gene_annotation() {
    ...
    awk -v awkvar="$bsample" -F "\t" 'BEGIN {FS=OFS="\t"} {print $0, awkvar}' "${vcf_file}-PASS" > "${final_output_folder}/${final_gatk_out}"

    awk -v awkvar="$bsample" -F "\t" 'BEGIN {FS=OFS="\t"} {print $0, awkvar}' "${vcf_file}-PASS-avinput.exonic_variant_function_WithGeneName" > "${final_output_folder}/${final_output_name}"
}
...
picard() {
    ...
    java -Xms4g -Xmx16g -jar "$EBROOTPICARD/picard.jar" AddOrReplaceReadGroups \
    I="${bam_file_folder}/${bsample}.bam" \
    O="${bam_file_folder}/${bsample}-rg_added_sorted.bam" SO=coordinate RGID=id RGLB=library \
    RGPL=platform RGPU=machine RGSM=${bsample}

    # Mark duplicates
    java -Xms4g -Xmx16g -jar "$EBROOTPICARD/picard.jar" MarkDuplicates \
    I="${bam_file_folder}/${bsample}-rg_added_sorted.bam" \
...
gatk_annovar() {
    ...
    -o "${bam_file_folder}/${bsample}-rg_added_sorted_dedupped_split.realigned.bam.filter.vcf"

    ### Annovar annotation to the GATK mutation calling
    annovar_gene_annotation \
    "${bam_file_folder}/${bsample}-rg_added_sorted_dedupped_split.realigned.bam.filter.vcf" \
...
## Steps for GATK mutation calling and then annotate gene with annovar
